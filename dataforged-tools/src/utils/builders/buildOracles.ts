...
export function buildOracles(game: Game = Game.Starforged): GameDataRoot["Oracle Sets"] {
  ...
  const oracleSetData = _.omitBy(concatWithYamlRefs<YamlOracleRoot>(REFS_PATH, ...oracleSetFiles), (v,k) => k.startsWith("_")) as {[key: string]: YamlOracleSet};
  ...
  const oracleJson = _(oracleSetData).mapValues((oracleSet) => new OracleSetBuilder(oracleSet, `${game}/Oracles`)).value()
