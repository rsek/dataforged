...
exports.toMdTable = exports.toRollColumnArray = exports.toResultColumnArray = exports.toSummaryColumnArray = void 0;
const renderOracleCategory_js_1 = require("./renderOracleCategory.js");
...
function toSummaryColumnArray(label, rows, minimumRows) {
    const rowContent = rows.map(row => row.Summary || "");
    return toColumnArray(label, rowContent, minimumRows);
}
exports.toSummaryColumnArray = toSummaryColumnArray;
function toResultColumnArray(label, rows, minimumRows) {
    const rowContent = rows.map(row => row.Result);
    return toColumnArray(label, rowContent, minimumRows);
}
exports.toResultColumnArray = toResultColumnArray;
function toRollColumnArray(label, rows, minimumRows) {
...
exports.toRollColumnArray = toRollColumnArray;
...
function mdTableFrom2dArray(array2d) {
    ...
    // inner borders
    const headerBorder = columnMax.map(columnWidth => lodash_es_1.default.repeat("-", columnWidth));
...
function buildMdOracles(headerText = "Ironsworn Oracles", ...oracleData) {
    ...
    const oracleMarkdown = oracleData.map(oracleCat => (0, renderOracleCategory_js_1.renderOracleCategory)(oracleCat), 2);
    return header + "\n\n" + oracleMarkdown.join("\n\n") + "\n";
}
const files = fs_1.default.readdirSync("./").filter(dir => dir.match(/^ironsworn_oracles/))
    .map(file => "./" + file);
const text = buildMdOracles(...files);
fs_1.default.writeFileSync("./oracles.md", text);
