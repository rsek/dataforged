// Code generated by jtd-codegen for Java + Jackson v0.2.1

package Dataforged;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonSerialize;

@JsonSerialize
public class MoveOutcomeMatchable {
    @JsonProperty("text")
    private MarkdownString text;

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty("count_as")
    private MoveOutcomeType countAs;

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty("match")
    private MoveOutcome match;

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty("reroll")
    private MoveReroll reroll;

    public MoveOutcomeMatchable() {
    }

    /**
     * Getter for text.<p>
     */
    public MarkdownString getText() {
        return text;
    }

    /**
     * Setter for text.<p>
     */
    public void setText(MarkdownString text) {
        this.text = text;
    }

    /**
     * Getter for countAs.<p>
     */
    public MoveOutcomeType getCountAs() {
        return countAs;
    }

    /**
     * Setter for countAs.<p>
     */
    public void setCountAs(MoveOutcomeType countAs) {
        this.countAs = countAs;
    }

    /**
     * Getter for match.<p>
     */
    public MoveOutcome getMatch() {
        return match;
    }

    /**
     * Setter for match.<p>
     */
    public void setMatch(MoveOutcome match) {
        this.match = match;
    }

    /**
     * Getter for reroll.<p>
     */
    public MoveReroll getReroll() {
        return reroll;
    }

    /**
     * Setter for reroll.<p>
     */
    public void setReroll(MoveReroll reroll) {
        this.reroll = reroll;
    }
}
